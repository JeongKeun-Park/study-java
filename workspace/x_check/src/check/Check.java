...
//		변수 : 저장공간(자료구조)
//		연산자 : 최단산쉬관리삼대
//		삼항 연산자 : 조건식이 1개만 있을때 사용
//		제어문 : 조건문(if문, switch문), 반복문(for, while, do~while)
//		배열 : 시작 주소, 저장공간 나열
//		메소드 : 이름 뒤에 소괄호, 주소를 담는 저장공간, 재사용, 소스코드 간결화
//		클래스 : 반, 타입이다, 주어이다
//		생성자 : 1. 메모리 할당 후 주소값 리턴, 2. 초기화, 오버 로딩 alt + shift + s , o
//		기본 생성자 : 매개변수가 없다. ctrl + space
//		객체화 : 추상적인 개념을 구체화 하는 작업 , 객체(intstance variable), .찍어서 접근
//		this : 접근한 객체 
...
//		반드시 기본 생성자를 만들어야 한다. 왜냐하면 자식 생성자에서는 기본적으로 부모의 기본 생성자를 호출하기 때문에.
//		접근자(pulic, default, protected, private)
...
//		Runnable resource = () -> {};
//		추상 클래스 : 강제성 , 추상 메소드 (abstract), 자식에서 반드시 재정의 한다.
//		일반 메소드도 선언 가능
//		인터페이스 : 강제성, 추상 메소드 밖에 없음. 상수밖에 안됨
//		함수형 인터페이스 : 추상 메소드가 1개만 있음. 구현할 때 이름 필요 없음, 람다식
//		람다식은 () -> {} , () -> 로 구현한다.
//		API : 제발 만들지말고 가져다 쓰자(Appkey, Example, 추가 다운로드 파일, 문해력 필요)
//		예외처리 : try~catch, Exception -> 항상오류, RuntimeException, throws -> Exception이 항상 뜰때, throw -> 터트릴때
